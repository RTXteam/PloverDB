...
PloverDB returns nodes and edges in TRAPI format in response to a query request, but for each node that
it returns, PloverDB does not include that node's "equivalent IDs" CURIEs. This is even though
PloverDB (under most types of graph index builds) has that information while it is building its 
indexes. The reason why PloverDB doesn't return equivalent node IDs with its query results is
for speed; lists of equivalent IDs can be quite lengthy. This information, furthermore, can
be easily obtained by the client directly by calling the NodeNorm API.
...
by a local modification of the Dockerfile).
...
| `/debug`                                                        | Custom        | GET          | Returns comprehensive diagnostic info: process uid/username, HOME directory, .git ownership, ownership match status, per-worker and system-wide memory usage, container cgroup limits. Essential for debugging ownership/permission issues on ITRB.On production systems, when not actively debugging, consider disabling this by defining (in the Dockerfile) the environment variable `PLOVER_DEBUG_SNAPSHOTS=false`, since having this as a public API command could provide an easy way for an attacker to DDoS the server. <br/><br/> **Example Queries:** <br/> `curl -X 'GET' 'https://multiomics.rtx.ai:9990/debug'`                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `labels`                         | object   | **Required**                 | Specifies the node and edge properties that Plover should treat as node and edge types when answering queries. The use of `labels` for this config file property name is admittedly a bit confusing (think of it like in Neo4j, where a node's "label" is its semantic type).  Defaults are `"category"` and `"predicate"`. <br/> **Example:** `{"edges": "predicate", "nodes": "all_categories"}`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `convert_input_ids`              | boolean  | **Strongly Recommended** |  Specifies whether Plover should build up a mapping of node CURIE identifiers to preferred identifiers, that it can use at query time to map identifiers in the query graph (which may not be preferred identifiers) to preferred identifiers. The source Plover uses for such equivalence mappings is either the graph itself (if PloverDB is configured to use them, via the `equiv_ids_in_graph` config option) it looks for node properties `same_as`, `equivalent_ids`, `equivalent_curies`, and `equivalent_identifiers`) or, if such a property is not present on nodes, the SRI [Node Normalizer](https://github.com/TranslatorSRI/NodeNormalization) API (via batch querying at build time). Default is `false`.  <br/> **Example:** `true`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `normalize`                      | boolean  | Optional             | Specifies whether Plover should canonicalize the underlying graph. If `true`, Plover will, at index-build time, remap the `subject` and `object` CURIE identifiers of each edge so that they are "preferred" identifiers (which is what Plover calls them). The mapping of identifiers to preferred identifiers that Plover uses for this step is build by Plover during the node-loading part of the index-building process. When PloverDB-hosting a Translator Data Ingest WG KGX-compliant knowledge graph, the graph's edges should already reference preferred identifiers, so `normalize` should be set to `false`. NOTE: if in the config file you set `normalize` to `true`, you need to also set `convert_input_ids` to `true` in the config file; but the converse is not true, so it is allowed to have `normalize` set to `false` _and_ `convert_input_ids` set to `true`. [Node Normalizer](https://github.com/TranslatorSRI/NodeNormalization) API. Default is `false`. <br/> **Example:** `true`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `drug_chemical_conflation`       | boolean  | Optional             | Specifies whether Plover should use the "drug_chemical_conflate" option when querying the SRI [Node Normalizer](https://github.com/TranslatorSRI/NodeNormalization) API. Default is `false`. When PloverDB-hosting a Translator Data Ingest WG KGX-compliant knowledge graph in which drugs/chemical and gene/protein conflation has already been done, this option (`drug_chemical_conflation`) should be set to `false`  <br/> **Example:** `true`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `is_test`                        | boolean  | Optional             | Specifies whether the graph to be built is a small "test graph" in which it is expected that some edges reference nodes that are not in the nodes file; this option is only ever set to `true` in development and never in production. Default is `false`.  <br/> **Example:** `true`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `debug`                          | boolean  | Optional             | Specifies whether to print extra debugging information (e.g., table of byte sizes of objects in memory) during the index-building step; this debugging information is very time-consuming to provide, so this option is only ever set to `true` during development work. Default is `false`.  <br/> **Example:** `true`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `equiv_ids_in_graph`       | boolean  | Optional             | This option tells PloverDB whether the input knowledge-graph provides all `same_as` edges that PloverDB will need in order to build up its mapping of node CURIE identifiers to preferred identifers. The `equiv_ids_in_graph` option is irrelevant if `convert_input_ids` is set to `false`; it only matters if `convert_input_ids` is set to `true`. Default is `true`. When PloverDB-hosting a Translator Data Ingest WG KGX-compliant knowledge graph that has full node normalization information in the KGX-compilant field (node property `same_as`), this option (`equiv_ids_in_graph`) should be set to `true` <br/> **Example:** `true`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
### Hosting a KGX-compliant, Babel-normalized, and conflated KG2 graph
For hosting in PloverDB, with Docker deployment, a KG2 graph (such as KG2.10.3) that 
1. complies with the format and schema for the "new KGX" format
as defined by the Translator Data Ingest Working Group
2. uses normalized identifiers on all node `id` fields and on all 
edge `subject` and `object` fields
3. has drug/chemical and gene/protein conflation already done
the config file options would be like this (specific KG2 version number and
Biolink version number will vary):
```
{
    "nodes_file": "https://kg2webhost.rtx.ai/kg2.10.3-conflated-nodes.jsonl.gz",
    "edges_file": "https://kg2webhost.rtx.ai/kg2.10.3-conflated-edges.jsonl.gz",
    ...
    "delete_local_kg_files": true,
    "normalize": false,
    "convert_input_ids": true,
    "equiv_ids_in_graph": true,
    "drug_chemical_conflation": false,
    "ignore_edge_properties": ["domain_range_exclusion", "publications_info"],
...
```
Note that if you ever are building PloverDB indexes _outside_ of Docker (which may be
useful during PloverDB development work), it is
very important to set `delete_local_kg_files` to `false`, or else PloverDB's `build_indexes`
code will delete your KG files after it builds its indexes.
